class SAKLauncher {
    ...
    loadApp(app) {
        ...
        // Aggiorna titolo
        appTitle.textContent = app.name.toUpperCase();

        // Cambia vista
        homeScreen.style.display = 'none';
        appViewer.style.display = 'flex';

        // Carica contenuto con sandbox più permissivo per download
        setTimeout(() => {
            const blob = new Blob([app.content], { type: 'text/html' });
            const url = URL.createObjectURL(blob);
            
            // Sandbox più permissivo per funzionalità complete
            appFrame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-downloads allow-modals allow-popups allow-presentation allow-top-navigation-by-user-activation');
            appFrame.src = url;
            
            appFrame.onload = () => {
                loading.style.display = 'none';
                appFrame.style.display = 'block';
                // Pulisci URL dopo caricamento
...
const launcher = new SAKLauncher();

// Imposta modalità home all'avvio
document.body.classList.add('home-mode');
document.querySelector('.container').classList.add('home-mode');